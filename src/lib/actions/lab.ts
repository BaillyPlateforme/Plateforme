"use server";

import { env } from "@/lib/env";

export interface LabEmail {
  from: string;
  subject: string;
  text: string;
}

// Envoie un email de test au webhook n8n (Lab bout-en-bout).
export async function sendLabEmail(email: LabEmail) {
  try {
    const res = await fetch(env.labWebhookUrl(), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        from: email.from,
        subject: email.subject,
        text: email.text,
        textPlain: email.text,
        receivedAt: "lab",
      }),
    });
    if (!res.ok) return { ok: false, message: `Webhook n8n : HTTP ${res.status}` };
    return { ok: true, message: "Envoyé au webhook ✓" };
  } catch (e) {
    return { ok: false, message: e instanceof Error ? e.message : "Erreur d'appel du webhook" };
  }
}

export async function sendLabBatch(emails: LabEmail[]) {
  const results = await Promise.all(emails.map((e) => sendLabEmail(e)));
  const ok = results.filter((r) => r.ok).length;
  return { ok, total: emails.length };
}
