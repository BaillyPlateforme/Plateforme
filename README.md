...
- **Claude** (`@anthropic-ai/sdk`) — analyse de volume par image (à venir)
...
```bash
cp .env.local.example .env.local   # puis remplir les clés Supabase + Anthropic
npm install
npm run dev
```
...
4. ⬜ Analyse volume IA (`/api/analyze-volume`, structured output Claude)
